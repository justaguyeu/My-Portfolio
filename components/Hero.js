...
// Extend Canvas with MeshLineGeometry and MeshLineMaterial
extend({ MeshLineGeometry, MeshLineMaterial });

useGLTF.preload('/YOU.glb');
useTexture.preload('/band2.jpg');

const Hero = () => {
  return (
    <div className="">
      <div>
        <Spotlight className="-top-90 -left-10 md:-left-32 md:-top-20 h-screen" fill="white" />
        <Spotlight className="h-[1000vh] w-[50vw] top-10 left-full" fill="purple" />
        <Spotlight className="left-80 top-28 h-[80vh] w-[50vw]" fill="blue" />
      </div>
      <div className="h-screen dark:bg-black bg-white dark:bg-grid-white/[0.03] bg-grid-black-100/[0.2] absolute top-0 left-0 flex items-center justify-center">
      ...
      <div className="flex flex-row items-center justify-center z-10">
        ...
        <a href="#main">
          <MagicButton title="Holla" icon={<FaArrowDownLong />} position="right" />
        </a>
      </div>
...
function Band({ maxSpeed = 50, minSpeed = 10 }) {
  ...
  const { nodes, materials } = useGLTF('/you.glb')
  const texture = useTexture('/band2.jpg')
